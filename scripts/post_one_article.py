...
未掲載のAIツール1件を1000〜1500字の見出し付き日本語記事に生成し、
...
SYSTEM_INSTRUCTION = (
    ...
    "2. 記事本文の合計文字数は1000〜1500文字を厳守すること。必ず1000文字以上、1500文字以内で書くこと。\n"
    "3. 箇条書きを一切使わず、読み物として深掘りした「解説文」形式で書くこと。\n"
    "4. 本文には必ず3〜4個のセクション見出しを入れること。見出しは「**見出しテキスト**」の形式で、"
    "前後を空行で挟んだ独立した1行として書くこと（例：\\n\\n**主要機能と他ツールとの違い**\\n\\n）。"
    "見出しテキストは検索キーワードを意識した具体的な文言にすること。\n"
    "5. 画像プロンプトは毎回、記事の具体的な文脈に基づいた新しい内容にすること。"
...
[本文]
以下の構成で合計1000〜1500文字の解説文を書くこと。箇条書き不可。流れるような文章で。
導入以外の各セクションの先頭には「**見出しテキスト**」形式の見出しを、前後を空行で挟んだ独立行として必ず入れること。
・導入（約150字・見出しなし）：このツールが解決する課題や背景を引き込む書き出しで。
・機能詳細（約500字）：見出し例「**{name}の主要機能と特徴**」。主要機能・特徴・他ツールとの差別化を深掘り解説。
・活用シーン（約300字）：見出し例「**どんな人・仕事に向いているか**」。どんな職種・業種・シーンで役立つか具体的に。
・料金・開始方法（約250字）：見出し例「**料金プランと始め方**」。無料プランの有無、料金体系、登録手順を簡潔に。
・まとめ（約150字）：見出し例「**まとめ：{name}はこんなツール**」。魅力を一言で締め、読者の行動を促す一文で終わる。
...
[本文]
合計1000〜1500文字の解説文。箇条書き不可。流れるような文章で。
本文中に3〜4個のセクション見出しを「**見出しテキスト**」形式（前後を空行で挟んだ独立行）で必ず入れること。
見出しは検索キーワードを意識した具体的な文言にすること（例：**なぜ今このニュースが重要なのか**）。
